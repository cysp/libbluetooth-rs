...
// < 01 05 0C 01 00
		...
		assert_eq!(b.to_bytes(), &[0x05, 0x0C, 0x01, 0x00]);
...
// < 01 01 0C 08 FF FF FB FF 07 F8 BF 3D
		...
		assert_eq!(b.to_bytes(), &[0x01, 0x0C, 0x08, 0xFF, 0xFF, 0xFB, 0xFF, 0x07, 0xF8, 0xBF, 0x3D]);
...
// < 01 01 20 08 1F 00 00 00 00 00 00 00
		...
		assert_eq!(b.to_bytes(), &[0x01, 0x20, 0x08, 0x1F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);

// > 04 0E 04 01 01 20 00
// < 01 02 10 00
		let oc = HciOpcode::Informational(HciInformationalOpcode::ReadLocalSupportedCommands);
		let b = HciCommandBuilder::new(oc).build();
		assert_eq!(b.to_bytes(), &[0x02, 0x10, 0x00]);
...
//   00 00 00 00 00
		...
		let b = HciCommandBuilder::new(oc)
			...
			.build();
		assert_eq!(b.to_bytes(), [0x52, 0x0C, 0xF1,
...
// < 01 04 10 01 01
		...
		assert_eq!(b.to_bytes(), &[0x04, 0x10, 0x01, 0x01]);
...
// < 01 12 0C 07 00 00 00 00 00 00 01
		...
		let b = HciCommandBuilder::new(oc)
			...
			.parameter(1u8)
			.build();
		assert_eq!(b.to_bytes(), &[0x12, 0x0C, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]);
...
// < 01 0F 08 02 0F 00
		...
		assert_eq!(b.to_bytes(), &[0x0F, 0x08, 0x02, 0x0F, 0x00]);
